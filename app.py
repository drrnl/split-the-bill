...
if st.session_state.stage == "upload":
    ...
    with tab2:
        ...
        with st.form("manual_item_form", clear_on_submit=True):
            ...
            if added and name:
                idx = len(st.session_state.items)
        ...
        if st.session_state.items:
            ...
            if st.button("Continue", type="primary", use_container_width=True):
                st.session_state.subtotal_detected = sum(
                    it["price"] for it in st.session_state.items
                )
                st.session_state.stage = "review"
                st.rerun()
...
# ============================================================
    ...
    if to_remove:
        st.session_state.items = [it for it in new_items if it["id"] not in to_remove]
        st.rerun()
    else:
    ...
    if st.button("+ Add item"):
        idx = len(st.session_state.items)
        st.session_state.items.append({"id": f"item_{idx}_{len(st.session_state.items)}", "name": "", "price": 0.0})
        st.rerun()

    st.divider()
    subtotal = sum(it["price"] for it in st.session_state.items)
    c1, c2 = st.columns(2)
    st.session_state.tax = c1.number_input(
...
# ============================================================
    ...
    unassigned = [it for it in st.session_state.items if not st.session_state.claims.get(it["id"])]
